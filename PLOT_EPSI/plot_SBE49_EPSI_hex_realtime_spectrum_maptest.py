...
def init_axes(fig):
    ...
    # set up subplot 
    ax0.cla()
    lineF0, = ax0.semilogy([],[], '.-', alpha=0.8, color="gray", markerfacecolor="red")
    lineV1, = ax0.semilogy([],[], '.-', alpha=0.8, color="gray", markerfacecolor="green")
    lineV2, = ax0.semilogy([],[], '.-', alpha=0.8, color="gray", markerfacecolor="cyan")
    radio = RadioButtons(ax1, ('Hanning', 'Top Hat')) 
    radio1 = RadioButtons(ax4, ('t1', 't2','s1','s2','a1','a2','a3')) 
    Sl_ymin = Slider(ax2, 'Ymin', -30, 0, valinit=-10)
    Sl_ymax = Slider(ax3, 'Ymax', -30, 0, valinit=-1)
    ax0.set_xlabel('Hz')
    ax0.set_ylabel('Volt^2/Hz')
    return lineF0,lineV1,lineV2,radio,radio1,Sl_ymin,Sl_ymax
...
def makefft(dataBuffer,Fs,radio):
  T        = dataBuffer.shape[0]
  V=1
  df       = Fs/T;                # Frequential resolution

  if radio.value_selected=='Hanning':
  	# hanning window to get rid of edges influence
  	hanning = 0.5 * (1-np.cos(2*np.pi*np.linspace(0,T-1,T)/(T-1)));
  	H=hanning;
  	wc=1/np.mean(np.array(hanning)**2);            # window correction factor
  else:
        #Sean you can make your own window H here and weight wc if needed
        H=1+dataBuffer*0
  data=np.array(H)*np.array(dataBuffer-np.mean(dataBuffer,0));# detrend  A2=A-mean(A)
...
def animate(i,radio,radio1,Sl_ymin,Sl_ymax):
    ...
    [k,spec]=makefft(Count2Volt_unipol(data),325.0,radio)
    ...
    lineF0.set_data(k,spec)
    #lineV1.set_data(EPSIsample.timeaxis-start_time,EPSIsample.timeaxis*0+Volt2Count_unipol(Vmax))
    #lineV2.set_data(EPSIsample.timeaxis-start_time,EPSIsample.timeaxis*0-Volt2Count_unipol(Vmin))
    lineV1.set_data(k,k*0+bitnoise_20(325))
    lineV2.set_data(k,k*0+bitnoise_24(325))
    ...
    fig.axes[0].set_ylim([10**ax0xmin,10**ax0xmax]) 
    #fig.axes[0].text(tmin1+.9*(tmax1-tmin1),ax0xmin+.9*(ax0xmax-ax0xmin),'rms=%3.3f' % (np.sqrt(np.mean(data**2))))  
    fig.axes[0].legend(['rms=%3.3f' % np.sqrt(np.mean(Count2Volt_unipol(data)**2))],loc=2)  
...
fig=init_figure(1)
lineF0,lineV1,lineV2,radio,radio1,Sl_ymin,Sl_ymax=init_axes(fig)
#ani = animation.FuncAnimation(fig, animate,frames=100,interval=10, blit=True)
ani = animation.FuncAnimation(fig, animate,fargs=(radio,radio1,Sl_ymin,Sl_ymax,),interval=1)
plt.show()
